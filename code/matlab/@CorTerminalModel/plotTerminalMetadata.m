function [fig1, fig2, fig3] = plotTerminalMetadata(self, T, binSrc)
    ...
    switch binSrc
        case 'geometry'
            % Distance
            bOwnDist = self.boundaries{strcmp(strrep(self.labels_initial, '"', ''), 'own_distance')};
            bIntDist = self.boundaries{strcmp(strrep(self.labels_initial, '"', ''), 'int_distance')};

            % Altitude
            bOwnAlt = self.boundaries{strcmp(strrep(self.labels_initial, '"', ''), 'own_alt')};
            bIntAlt = self.boundaries{strcmp(strrep(self.labels_initial, '"', ''), 'int_alt')};

            % Speed
            bOwnSpd = self.boundaries{strcmp(strrep(self.labels_initial, '"', ''), 'own_speed')};
            bIntSpd = self.boundaries{strcmp(strrep(self.labels_initial, '"', ''), 'int_speed')};

        case 'trajectory'
            % Distance
            bOwnDist = self.mdlFwd1_1.boundaries{strcmp(strrep(self.mdlFwd1_1.labels_initial, '"', ''), 'distance')};
            bIntDist = self.mdlFwd2_1.boundaries{strcmp(strrep(self.mdlFwd2_1.labels_initial, '"', ''), 'distance')};

            % Altitude
            bOwnAlt = self.mdlFwd1_1.boundaries{strcmp(strrep(self.mdlFwd1_1.labels_initial, '"', ''), 'altitude')};
            bIntAlt = self.mdlFwd2_1.boundaries{strcmp(strrep(self.mdlFwd2_1.labels_initial, '"', ''), 'altitude')};

            % Speed
            bOwnSpd = self.mdlFwd1_1.boundaries{strcmp(strrep(self.mdlFwd1_1.labels_initial, '"', ''), 'speed')};
            bIntSpd = self.mdlFwd2_1.boundaries{strcmp(strrep(self.mdlFwd2_1.labels_initial, '"', ''), 'speed')};
        otherwise
    ...
    ax1 = nexttile;
    histogram(T.own_distance_nm, bOwnDist, 'Normalization', 'probability', ownStyle{:});
    hold on;
    histogram(T.int_distance_nm, bIntDist, 'Normalization', 'probability', intStyle{:});
    hold off;
    ...
    % Altitude
    ax2 = nexttile;
    histogram(T.own_alt_ft, bOwnAlt, 'Normalization', 'probability', ownStyle{:});
    hold on;
    histogram(T.int_alt_ft, bIntAlt, 'Normalization', 'probability', intStyle{:});
    hold off;
    ...
    % Speed
    ax3 = nexttile;
    histogram(T.own_speed_ftps, bOwnSpd, 'Normalization', 'probability', ownStyle{:});
    hold on;
    histogram(T.int_speed_ftps, bIntSpd, 'Normalization', 'probability', intStyle{:});
    hold off;

    set(gca, 'XTick', bOwnSpd, 'XTickLabelRotation', 45);
    xlabel('Feet per second');
    grid on;
    title('Speed');
    legend('Location', 'northeast');
    ...
    % Vertical Rate
    ax4 = nexttile;
    histogram(T.own_vertRate_ftpm / 60, bVertRate, 'Normalization', 'probability', ownStyle{:});
    hold on;
    histogram(T.int_vertRate_ftpm / 60, bVertRate, 'Normalization', 'probability', intStyle{:});
    hold off;
    ...
    colormap parula;
    caxis([0 1]); % colorbar; colorbar('Ticks',0:.1:1,'Limits',[0 1]);
